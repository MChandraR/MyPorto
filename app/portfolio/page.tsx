...
export default function PortfolioPage() {
  ...
  const filteredProjects = projects.filter((project) => {
    const projectTitle = language === "en" ? project.titleEn : project.titleId;
    const projectDesc = language === "en" ? project.descriptionEn : project.descriptionId;

    const matchesSearch =
      projectTitle.toLowerCase().includes(search.toLowerCase()) ||
      project.tech.some((t) => t.toLowerCase().includes(search.toLowerCase())) ||
      projectDesc.toLowerCase().includes(search.toLowerCase());
      ...
      {filteredProjects.length > 0 ? (
        ...
        <div className="text-center py-20 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl">
          <p className="text-zinc-500 dark:text-zinc-500 text-sm">
            {t("port.empty").replace("{search}", search).replace("{tab}", getTabLabel(activeTab))}
          </p>
        </div>
      )}
    </main>
